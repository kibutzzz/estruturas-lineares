...
class ListarProdutosAbaixoMinimoTest {

    @InjectMocks private ListarProdutosAbaixoMinimo acao;
    ...
    void deveMostrarTabelaDeProdutos() {
        ...
        estoque.adicionarProduto(new Produto("2", "Feijão", 2, 5));
        ...
        then(logger)
                .should(order)
                .imprimirLinha("         2 -          Feijão -          2 -          5");
        then(logger).shouldHaveNoMoreInteractions();
    }

    @Test
    void deveTerNomeCorreto() {
        assertEquals("Listar produtos abaixo do estoque minimo", acao.getNome());
    }
}
